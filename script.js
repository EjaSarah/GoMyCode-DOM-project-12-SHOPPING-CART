...
function updateUi() {
  for (let good in goods) {
   ...
   `;
    ...
    //plus minus, liked and delete button for each items mapped
    ...
    const deleteIcon = cardBody.querySelector("#delete-icon"); //heart

    plus.addEventListener("click", () => add(goods[good]));

    minus.addEventListener("click", () => add(goods[good]));
    ...
    deleteIcon.addEventListener("click", () => {
      if (goods[good].isdeleted) {
        goods[good].isdeleted = false;
        deleteIcon.good.color = "red";
      } else {
        goods[good].isLiked = true;
